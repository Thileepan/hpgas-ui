...
$(document).ready(async function() {  // ← Made async
    ...
    $('#customerForm').submit(async function(e) {  // ← Made async
        e.preventDefault();
        await saveCustomer();  // ← Added await
    });
});

async function loadCustomers() {  
    const user = getCurrentUser();
    // Super admin sees all customers, manager sees only their godown's customers
    allCustomers = user.role === 'super_admin' 
        ? await getCustomers() 
        : (await getCustomers()).filter(c => c.godown_id === user.godown_id);
    filterCustomers();
    updateStats();
}
...
async function displayCustomers() {  // ← Made async
    ...
    // Get deliveries for history - UPDATED: Made async
    ...
    filteredCustomers.forEach((customer, index) => {
        ...
        // Get delivery count for customer
        const customerDeliveries = deliveries.filter(d => d.customer_id === customer.id);
        const deliveryCount = customerDeliveries.length;
        ...
        html += `
            <div class="card animate-slide-up" style="animation-delay: ${index * 0.05}s">
                ...
                <div class="space-y-2 mb-4">
                    ...
                    ${deliveryCount > 0 ? `
                        <div class="flex items-center gap-2 text-sm text-gray-600">
                            ...
                            <span>${deliveryCount} deliveries</span>
                        </div>
                    ` : ''}
                </div>
                ...
                <div class="flex gap-2 pt-3 border-t border-gray-200">
                    ...
                    <button onclick="deleteCustomer(${customer.id}, '${customer.name.replace(/'/g, "\\'")}')" class="btn btn-secondary btn-sm text-red-600 hover:bg-red-50">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function getBusinessTypeIcon(type) {
    const icons = {
        ...
        'restaurant': '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>',
        'hospital': '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>',
        'school': '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path>',
        'catering': '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 15.546c-.523 0-1.046.151-1.5.454a2.704 2.704 0 01-3 0 2.704 2.704 0 00-3 0 2.704 2.704 0 01-3 0 2.704 2.704 0 00-3 0 2.704 2.704 0 01-3 0 2.701 2.701 0 00-1.5-.454M9 6v2m3-2v2m3-2v2M9 3h.01M12 3h.01M15 3h.01M21 21v-7a2 2 0 00-2-2H5a2 2 0 00-2 2v7h18zm-3-9v-2a2 2 0 00-2-2H8a2 2 0 00-2 2v2h12z"></path>',
...
async function saveCustomer() {  // ← Made async
    const user = getCurrentUser();
    ...
    let customers = await getCustomers();  // ← Added await
...
async function deleteCustomer(id, name) {  // ← Made async
    if (!confirm(`Are you sure you want to delete customer "${name}"?\n\nThis action cannot be undone.`)) {
        return;
    }

    // Check if customer has any deliveries - UPDATED: Made async
    const deliveries = await getDeliveries();
...
async function viewCustomerDetails(id) {  // ← Made async
    const customer = allCustomers.find(c => c.id === id);
    if (!customer) return;
    ...
    // Get delivery history - UPDATED: Made async
    const deliveries = await getDeliveries();
    const customerDeliveries = deliveries.filter(d => d.customer_id === id);
    ...
    if (customerDeliveries.length > 0) {
        const sortedDeliveries = customerDeliveries.sort((a, b) => 
            new Date(b.delivered_at || b.created_at) - new Date(a.delivered_at || a.created_at)
        );
        const lastDeliveryDate = new Date(sortedDeliveries[0].delivered_at || sortedDeliveries[0].created_at);
        lastDelivery = lastDeliveryDate.toLocaleDateString();
    }
    ...
    const html = `
        <div class="space-y-6">
                ...
                <div class="flex-1">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 mt-1">${customer.business_type}</p>
                        </div>
                        ${statusBadge}
                    </div>
                </div>
            ...
            <div class="pt-6 border-t border-gray-200">
                <h4 class="text-md font-semibold text-gray-900 mb-3">Location</h4>
                <div class="space-y-2">
                    ...
                    ${customer.latitude && customer.longitude ? `
                        <div class="flex items-center gap-3">
                            ...
                            <a href="https://www.google.com/maps?q=${customer.latitude},${customer.longitude}" target="_blank" class="text-sm text-orange-600 hover:text-orange-700">View on Map</a>
                        </div>
                    ` : ''}
                </div>
            </div>
